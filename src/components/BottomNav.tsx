...
import { ArrowLeft, LayoutDashboard, BookOpen, Star, Backpack, Settings, User, GraduationCap, NotebookPen } from 'lucide-react';

type TabKey = 'home' | 'spells' | 'features' | 'attributes' | 'inventory' | 'biography' | 'notes' | 'settings';
...
export function BottomNav({ activeTab, setActiveTab, onDashboard }: Props) {
  ...
  return (
    <nav
      className={`fixed left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-md z-40 ${t.navBg} border ${t.navBorder} rounded-2xl p-2 shadow-lg ${t.cardShadow} flex justify-between items-center backdrop-blur-sm`}
